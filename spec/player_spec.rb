...
describe Player do
  describe 'Player.new' do
    ...
    it "players 1 chooses 'Name' of atleast 1 letter " do
      expect(player_one.name.length).to be > 0
      expect(player_one.name.length).not_to be > 16
    end
    ...
    it 'players moves-array limits length of 9' do
      ...
      expect(player_one.moves_arr.length).not_to be < 0
    end

    it 'players moves-array all values are set to 0' do
      expect(player_one.moves_arr).to eql(Array.new(9, 0))
      expect(player_one.moves_arr).not_to eql(Array.new(9, (1..10)))
    end
  end
end
